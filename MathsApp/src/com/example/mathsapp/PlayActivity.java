...
public class PlayActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		voiceControlButton = (Button) findViewById(R.id.voice_control);
    ...
    public void return__button(int total) 
    {
    	String more_than_6 = "恭喜!恭喜! "+"答对的了"+total+"道题哦";
    	String less_than_6 = "很遗憾!哟~"+"只答对的了"+total+"道题哦";
    	if(total > 6)
    	{
    		new AlertDialog.Builder(this).setTitle(more_than_6).setIcon(android.R.drawable.ic_dialog_info)
    	...
    	{
    		new AlertDialog.Builder(this).setTitle(less_than_6).setIcon(android.R.drawable.ic_dialog_info)
    		.setPositiveButton("确定", new DialogInterface.OnClickListener() {
    			@Override
    			public void onClick(DialogInterface dialog, int which) {
    				// 点击“确认”后的操作
    ...
    public void voice_on()
	{
		if(mediaPlayer==null)
		{
			mediaPlayer = MediaPlayer.create(this, R.raw.bg_mic);
			mediaPlayer.start();
		}
		/*
		if(soundPool==null)
	...
	public void Button_control_voice()
	{
		voiceControlButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				{
					voiceControlButton.setBackgroundDrawable(getResources().getDrawable(R.drawable.voice_stop));
					voice_off();
					buttonIsPressed=true;
				}
				else
				{
					voiceControlButton.setBackgroundDrawable(getResources().getDrawable(R.drawable.voice));
					voice_on();
	...
	protected void onResume() {
		...
		if(mediaPlayer == null)
		{
			mediaPlayer=MediaPlayer.create(PlayActivity.this, R.raw.playmusic);
		}
		mediaPlayer.start();
		...
		for (int i = 0; i < 10; i++) {//10道题
			...
			if (ccs == 0) {// 一般式子
				...
				for (int k = 0; k < n - 1; k++) {
					...
					if (no[k] == 3 && num1[k + 1] == 0) { //******操作符为除号 并且分母为0
						do {
							num1[k + 1] = (int) (Math.random() * 100);// ***分母重新取数
						} while (num1[k + 1] == 0);
					}
				}
				for (int h = 0; h < n; h++) {//负数加括号
					if (h != n - 1) { //****判断数是否最后一个
						if (num1[h] < 0) //****如果为负数加括号加操作符
